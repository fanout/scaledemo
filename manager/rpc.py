...
class RpcServer(object):
	def __init__(self, rep_sock_specs, bind=True, context=None):
		if context:
			self.context = context
		else:
			self.context = zmq_context
		...
		if bind:
			self.rep_sock.bind(rep_sock_specs[0])
		else:
			for spec in rep_sock_specs:
				self.rep_sock.connect(spec)
		self.req_id = None
		for spec in rep_sock_specs:
			if spec.startswith('ipc://'):
				os.chmod(spec[6:], 0o777)
